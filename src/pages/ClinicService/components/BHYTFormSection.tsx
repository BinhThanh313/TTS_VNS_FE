...
export const BHYTFormSection: React.FC<Props> = ({ form }) => {
  ...
  return (
    <div className="mt-6 p-4 border border-gray-300 rounded-lg">
      <h3 className="text-blue-500 mt-0 mb-4 font-semibold text-lg">Thông tin BHYT</h3>
      <Row gutter={16}>
      ...
      <Row gutter={16} align="middle">
        ...
        <Col span={5}><Form.Item name="giaDV" label="Giá DV" rules={[{ required: true }]}><InputNumber className="w-full" min={0} formatter={v => `${v}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} /></Form.Item></Col>
        <Col span={5}><Form.Item name="giaBHYT" label="Giá BHYT"><InputNumber className="w-full" min={0} disabled={!isOpenBHYT} /></Form.Item></Col>
        <Col span={5}><Form.Item name="bhytChiTra" label="BHYT chi trả"><InputNumber className="w-full" min={0} disabled={!isOpenBHYT} /></Form.Item></Col>
        <Col span={5}><Form.Item name="tiLeBHYTChiTra" label="Tỉ lệ chi trả (%)"><InputNumber className="w-full" min={0} max={100} disabled={!isOpenBHYT} /></Form.Item></Col>
      </Row>

      <h4 className="mt-4 mb-3 font-medium">Danh sách giá</h4>
      <Form.List name="lichSuGia">
        {(fields, { add, remove }) => (
          <>
            {fields.map(({ key, name, ...restField }) => (
              <Space key={key} className="flex mb-2 items-baseline">
                ...
                <AppButton type="text" danger icon={<Trash2 size={16} />} onClick={() => remove(name)} />
              </Space>
            ))}
            <Form.Item>
              <AppButton type="dashed" onClick={() => add()} block icon={<Plus size={16} />}>
                Thêm thông tin giá
              </AppButton>
            </Form.Item>
          </>
