...
// Kích hoạt plugin để parse định dạng ddmmyyyy
dayjs.extend(customParseFormat);

const { Title, Text } = Typography;

// --- DỮ LIỆU GIẢ LẬP (MOCK DATA) THEO TÀI LIỆU ---
const mockChartData = [
...
export default function RevenueReport() {
  ...
  // LOGIC TÌM KIẾM VÀ KIỂM TRA 1 THÁNG
  const handleSearch = () => {
    if (!fromDate || !toDate) {
  ...
  const formatYAxis = (tickItem: any) => typeof tickItem === 'number' ? (tickItem / 1000000).toString() + 'M' : tickItem;
        ...
        <Col span={14}>
          <Card title={<div style={{ textAlign: 'center', fontWeight: 'bold', color: '#555' }}>Doanh thu theo hình thức thanh toán</div>}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              ...
              <Select defaultValue="tienMat" style={{ width: 150 }} onChange={(val) => setPaymentType(val)}>
                <Select.Option value="tienMat">Tiền mặt</Select.Option>
                <Select.Option value="qrDong">QRMB Động</Select.Option>
                <Select.Option value="qrTinh">QRMB Tĩnh</Select.Option>
                <Select.Option value="pos">POS</Select.Option>
                <Select.Option value="khac">Chuyển khoản khác</Select.Option>
              </Select>
            </div>
  ...
  return (
    <div style={{ padding: '0 24px 24px 24px', background: '#fff', minHeight: '100%' }}>
      {/* BỘ LỌC TÌM KIẾM ĐÃ TÍCH HỢP VALIDATE */}
      ...
      <Tabs 
        ...
        style={{ marginTop: 16 }}
        tabBarExtraContent={
          activeTab !== '1' ? <Button type="primary" icon={<DownloadOutlined />}>Xuất Excel</Button> : null
        }
        items={[
